...
void GoUp(int32 isDown, int32 wasDown)
{
	cameraPositionY += 20.0f;
}

void GoDown(int32 isDown, int32 wasDown)
{
	cameraPositionY -= 20.0f;
}
...
INT CALLBACK wWinMain(HINSTANCE instance, HINSTANCE previousInstance, PWSTR commandLine, int showCode)
{
	...
	if (window)
	{
		...
		keyboardInput.key = 'W';
		RegisterKeyboardInput(keyboardInput, GoUp);
		keyboardInput.key = 'S';
		RegisterKeyboardInput(keyboardInput, GoDown);
